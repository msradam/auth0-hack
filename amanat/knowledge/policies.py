...
def get_documents_for_prompt(policy_ids: list[int] | None = None) -> str:
    ...
    Uses compact format: only doc_id, title, and first 200 chars of text
    to keep the prompt short for small LLMs. Full text is available in the
    violations themselves (the rules engine cites policies by title).
    """
    import json
    docs = POLICIES if policy_ids is None else [p for p in POLICIES if p["doc_id"] in policy_ids]
    if not docs:
        return ""
    lines = []
    for doc in docs:
...
def search_policies(query: str, max_results: int = 5) -> list[dict]:
    """Keyword search over policy documents. Returns matching policies.

    For scan-type queries, returns fewer results to keep the system prompt
    short (important for small local LLMs like Granite Micro).
    For policy questions, returns more detailed results.
    """
